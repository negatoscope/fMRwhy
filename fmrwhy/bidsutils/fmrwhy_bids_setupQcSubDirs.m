...
if ~exist(options.sub_dir_preproc, 'dir')
    ...
    for i = 1:numel(dir_contents)
        % If sessions exist
        if (dir_contents(i).isdir == 1) && strfind(dir_contents(i).name, 'ses')
            sesdir_contents = dir(fullfile(options.sub_dir_BIDS, dir_contents(i).name));
            sesdir_file = fullfile(options.sub_dir_BIDS, dir_contents(i).name)
            % Loop through session dir
            for j = 1:numel(sesdir_contents)
                % if there are anat or func directories, copy/gunzip files accordingly
                if (sesdir_contents(j).isdir == 1) && (strfind(sesdir_contents(j).name, 'anat') || strfind(sesdir_contents(j).name, 'func'))
                    anatfunc_dir = fullfile(sesdir_file, sesdir_contents(j).name);
        ...
        elseif (dir_contents(i).isdir == 1) && (strfind(dir_contents(i).name, 'anat') || strfind(dir_contents(i).name, 'func'))
            anatfunc_dir = fullfile(options.sub_dir_BIDS, dir_contents(i).name);
...
function copyGunzipDirContents(srcDir, destDir)
    ...
    for i = 1:numel(srcDir_contents)
        srcDir_file = fullfile(srcDir, srcDir_contents(i).name);
        if strfind(srcDir_contents(i).name, 'sub')
            if strfind(srcDir_contents(i).name, 'nii.gz')
